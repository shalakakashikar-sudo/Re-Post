...
const CATEGORY_METADATA: Record<TopicCategory, { description: string, icon: string }> = {
  'Foundations': { description: 'The absolute basics. Identify sentence types and understand the core transformation framework.', icon: '🏗️' },
  'Statements': { description: 'Master tense backshifts, pronoun alignment, and time/place adjustments for declarations.', icon: '✉️' },
  'Questions': { description: 'Learn to handle Yes/No and Wh-inquiries with word-order flips and specific connectors.', icon: '❓' },
  'Imperatives': { description: 'Convert orders, requests, and advice using the infinitive "to" structure.', icon: '❗' },
  'Exclamations': { description: 'Capture strong emotions by mapping interjections to descriptive reporting verbs.', icon: '🎉' },
  'Advanced': { description: 'Tackle complex scenarios like conditionals, mixed sentences, and layered reporting.', icon: '🖇️' },
  'Mastery': { description: 'The ultimate challenge. Reverse transformations and expert-level error diagnosis.', icon: '🏆' }
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Scroll to top of the page when changing major sections
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [section]);

  useEffect(() => {
    // Scroll to the main content area when a topic is selected
    if (selectedTopic && mainContentRef.current) {
  ...
  const resetQuiz = (msg = "Starting the training session!") => {
    setUserAnswers({});
    setQuizState({
      currentIndex: 0,
      score: 0,
  ...
  const handleStartTopicQuiz = (topic: LearnTopic) => {
    ...
    resetQuiz(`Module ${topic.moduleId} Quiz Initiated!`);
    setSection('practice');
  };
  ...
  const handleStartGrandExam = () => {
    ...
    resetQuiz("Grand Evaluation: 20 Questions!");
  ...
  const handleAnswer = (option: string) => {
    ...
    setQuizState(prev => ({
      ...
      feedback: isCorrect ? "Correct! Rule applied perfectly." : "Incorrect! Let's check the logic.",
        ...
        <div className="flex flex-col items-center justify-center animate-fadeIn py-8 min-h-[70vh]">
          <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div className="text-center md:text-left space-y-6">
              <div className="inline-block bg-red-50 text-red-600 px-4 py-1 rounded-full font-black text-xs tracking-widest uppercase mb-2 animate-bounce">Grammar Mastery Path</div>
              ...
              <p className="text-gray-600 text-xl font-medium max-w-lg leading-relaxed">
                Learn to transform speech with <span className="text-orange-600 font-black underline decoration-wavy">Waffle the Grammar Hamster</span> through 30 intensive levels of English syntax.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 pt-6">
                <button onClick={() => setSection('learn')} className="bg-red-600 text-white px-10 py-5 rounded-[2.5rem] font-black shadow-2xl hover:bg-red-700 transition-all text-xl uppercase tracking-widest border-b-8 border-red-900 active:translate-y-2 active:border-b-0">Begin Learning</button>
            ...
            <div className="flex justify-center">
              <Waffle dialogue="Squeak! Ready for a syntax session?" mood="excited" size="lg" />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-12 animate-fadeIn pb-20">
          <div className="bg-white p-8 rounded-[3.5rem] border-4 border-blue-50 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8 border-dashed">
            <div className="text-center md:text-left">
              ...
              <p className="text-red-500 font-black uppercase tracking-[0.3em] text-xs mt-2">Comprehensive 30-Level Curriculum</p>
            </div>
            <Waffle dialogue="Select a category to view modules." mood="thinking" size="sm" />
          </div>
      ...
      {selectedTopic && section === 'learn' && (
        <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto pb-20">
          <div className="flex items-center justify-between bg-white p-6 rounded-[3rem] border-4 border-blue-50 shadow-xl sticky top-20 z-40">
            <button onClick={() => setSelectedTopic(null)} className="bg-gray-100 px-6 py-2 rounded-full text-blue-900 font-black hover:bg-gray-200 text-sm uppercase">← Back</button>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            <div className="lg:col-span-3 space-y-8">
              <div className="bg-white p-8 rounded-[3rem] shadow-2xl border-2 border-orange-100 relative overflow-hidden">
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-2xl border-4 border-dashed border-gray-200 text-center space-y-8">
                <div className="space-y-3">
                  <p className="text-[10px] uppercase text-gray-400 font-black tracking-[0.3em]">Direct Speech Format</p>
                ...
                <div className="space-y-3">
                  <p className="text-[10px] uppercase text-green-500 font-black tracking-[0.3em]">Indirect Speech Result</p>
            ...
            <div className="lg:col-span-2 space-y-8">
               ...
               <button onClick={() => handleStartTopicQuiz(selectedTopic)} className="w-full bg-red-600 text-white py-6 rounded-[2.5rem] font-black shadow-2xl hover:bg-red-700 text-xl tracking-widest border-b-8 border-red-900 active:translate-y-2 transition-all uppercase">Start Practice</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="animate-fadeIn max-w-5xl mx-auto space-y-8 pb-20">
          {!quizState.finished ? (
              ...
              <div className="lg:col-span-3 space-y-8">
                <div className="bg-white p-10 rounded-[4rem] shadow-3xl border-4 border-blue-50 relative min-h-[500px] flex flex-col justify-center">
                  <div className="absolute top-8 left-10 flex items-center gap-4">
                    ...
                    <span className="text-gray-400 font-black uppercase text-xs tracking-[0.3em]">Module Challenge</span>
                  </div>
                  <div className="space-y-12">
                    <div className="space-y-4">
                      <p className="text-[10px] font-black text-red-500 uppercase tracking-[0.4em] text-center">Transform the following:</p>
                      ...
                      {quizQuestions[quizState.currentIndex]?.options.map((opt, i) => {
                        ...
                        return (
                          <button key={i} disabled={isAnswered} onClick={() => handleAnswer(opt)} className={`p-6 rounded-[2rem] border-4 text-left font-black text-lg transition-all flex items-center gap-6 ${isAnswered ? (isCorrect ? 'bg-green-100 border-green-500 text-green-900' : isSelected ? 'bg-red-100 border-red-500 text-red-900' : 'opacity-40 grayscale blur-[1px]') : 'bg-white border-gray-100 hover:border-blue-500 hover:-translate-y-1'}`}>
                            <span className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-2xl shrink-0 ${isAnswered && isCorrect ? 'bg-green-600 text-white' : 'bg-blue-50 text-blue-600'}`}>
                ...
                <div className="flex justify-between items-center bg-white p-6 rounded-[3rem] shadow-2xl border-4 border-blue-50">
                   ...
                   <button onClick={handleNext} disabled={!userAnswers[quizState.currentIndex]} className="px-10 py-4 bg-red-600 text-white rounded-[2rem] font-black shadow-2xl hover:bg-red-700 disabled:opacity-50 transition-all uppercase tracking-[0.2em]">Next →</button>
                </div>
              </div>
              <div className="space-y-8">
                <Waffle dialogue={quizState.feedback || "Apply the transformation rules!"} mood={quizState.mood} />
                {userAnswers[quizState.currentIndex] && (
                  <div className="bg-white p-8 rounded-[3rem] border-4 border-blue-50 shadow-2xl animate-fadeIn space-y-4">
                    <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest border-b-2 border-blue-50 pb-2">Grammar Explanation</p>
            ...
            <div className="bg-white p-12 rounded-[5rem] shadow-3xl text-center space-y-10 animate-scaleIn border-[16px] border-blue-50 max-w-3xl mx-auto py-16 relative overflow-hidden">
              <h2 className="text-7xl font-black text-blue-900 leading-none">Module Complete!</h2>
              <div className="flex gap-6 justify-center">
                 <div className="bg-blue-50 p-10 rounded-[3.5rem] flex-1 border-4 border-blue-100">
              ...
              <Waffle dialogue="Excellent progress. Review the roadmap to continue your journey." mood="happy" size="md" />
              <button onClick={() => setSection('home')} className="w-full bg-red-600 text-white py-6 rounded-[3rem] font-black shadow-2xl hover:bg-red-700 text-2xl uppercase">Home</button>
            </div>
          )}
        </div>
      )}
